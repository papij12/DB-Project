...
namespace SqlDatabase
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            NpgsqlCommand cmd2 = new NpgsqlCommand("Select m. movie_id,title,first_name,last_name,price From movies m Join starring s On m.movie_id = s.movie_id Join actors a On s.actor_id = a.actor_id Where m.movie_id = @movie_id", conn);
            cmd2.Parameters.AddWithValue("@movie_id", int.Parse(movie_id));
            NpgsqlDataReader dataReader1 = cmd2.ExecuteReader();
            while (dataReader1.Read())
            {
                Console.WriteLine($" movie {dataReader1["movie_id"]} has title\"{dataReader1["title"]}\"actor first name: {dataReader1["first_name"]}\"\t actor last name : {dataReader1["last_name"]}\"  cost {dataReader1["price"]}");
            }
